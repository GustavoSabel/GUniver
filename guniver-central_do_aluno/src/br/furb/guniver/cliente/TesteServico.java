...
public class TesteServico {
	...
	private static void TestarCentralAluno() {
		try {
			...
			compromisso.setDataInicio(dateToXMLGregorianCalendar(calendar
					.getTime()));

			calendar.add(Calendar.HOUR, 24);
			compromisso.setDataFim(dateToXMLGregorianCalendar(calendar
					.getTime()));

			compromisso = port.criarCompromisso(compromisso);
			System.out.println("Cadastrado o compromisso "
					+ compromisso.getCodigo());
			...
			System.out
					.println("Todos compromissos cadastrados para o aluno 1:");
			for (Compromisso compromisso2 : compromissos) {
				System.out.println(" - " + compromisso2.getCodigo() + " - "
						+ compromisso2.getTitulo());
			}
			...
			System.out.println("Remover o compromisso "
					+ compromisso.getCodigo());
	...
	private static void TestarIntegracaoCadastro() {
		try {
			...
			port.getDisciplinas(1);
			System.out.println("Consultou Disciplinas");
			port.getTurmas(1);
			System.out.println("Consultou Turmas");
			br.furb.guniver.cliente.stubs.Aluno aluno = port.autenticaUsuario(
					"Teste", "1234");
			if (aluno == null)
		...
		try {
			...
			for (Matricula matricula : matriculas) {
				System.out.println(" - Aluno: "
						+ matricula.getAluno().getNome() + " - Semestre:"
